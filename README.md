...
Если проект лежит внутри внешней папки с длинным именем (например после распаковки архива) и внутри есть каталог **`Labs`**: откройте в IDE именно **`Labs`** — там `pom.xml`, `mvnw` и **корень Git**; команды `git` и `./mvnw` выполняйте из этой папки. Имя внешней директории на работу Spring Boot не влияет, в коде путей к ней нет.

**Десктоп-практики (задание 2.x, C++/Win32)** вынесены в **отдельный репозиторий Git** — см. локальную папку **`PracticaPractice`** рядом с этим проектом (или свой remote по заданию преподавателя).
